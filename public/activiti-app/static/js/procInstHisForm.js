...
var myTabContent = new Vue({
    ...
    mounted: function () {
        // 消除不满窗时的滚动条
        if ($("#procInstHisForm").height() < $("#procInstHisForm").context.defaultView.innerHeight) {
            $($("#procInstHisForm").context.firstElementChild).height(
...
window.operateEvents = {
    'click .viewDetail': function (e, value, row, index) {
        var formUrl = getRelativePath() + "businessTestForm.html" + "?procInstId=" + row.procInstId
            + "&userTaskInstId=" + row.id + "&state=" + row.state
            + "&dealUserId=" + row.dealUserId + "&editType=view";
