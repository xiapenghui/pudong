	...
	methods: {
		onClickNum(obj) {
			...
			if (obj == 'fkjc') {
				...
				eventBus.emit("layer-addLayer-CableVibrationLayer", true);//光纤振动告警-地图
				this.dwTip = 'ycgx';
				...
				this.$$utils
					.axiosRequest(
						'/pdaqfh_Web_exploded/f/xczy/tXczyWarnMessage/getTXczyWarnMessageList?types=das_alarm&rangeType=today&offset=' + this.offset + '&limit=' + this.pageSize,
						'获取异常光纤设备列表'
					)
					.then(result => {
		...
		onhandClick(row) {
			...
			if (this.dwTip == 'znjg') {
				...
				let layer = {
					id: row.device_id,  //定位对象id
					layerName: 'CableVibrationLayer', //定位图层
					zoom: 18, //定位图层
					point: {
						lng: point[0],
						lat: point[1]
		...
		// 获取异常数据的接口
		delayed() {
			// '/pdaqfh_Web_exploded/f/xczy/tXczyWarnMessage/getTodayWarnMessageList', '获取今日告警消息列表'
			// '/pdaqfh_Web_exploded/f/xczy/tXczyWarnMessage/getTXczyWarnMessageList?rangeType=month&offset=0&limit=5',
			//			'获取异常井盖设备列表'
			this.$$utils
				.axiosRequest('/pdaqfh_Web_exploded/f/xczy/tXczyWarnMessage/getTXczyWarnMessageList?rangeType=today&offset=0&limit=-1', '获取异常井盖设备列表')
				.then(result => {
					...
					result.data.list.map(item => {
						if (item.type == 'znjg_qxgj') {
							this.ycjgNumber++;
							abnormalWell.push(item);
						}
						if (item.type == 'das_alarm') {
							this.ycgxNumber++;
							abnormalOptical.push(item);
						}
					});
					//将异常光纤和异常井盖的数据合并在一起 便于弹窗展示
					let totalOptical = abnormalWell.concat(abnormalOptical);
					//获取本地存储的异常光纤和异常井盖数据
					let localOptical = JSON.parse(localStorage.getItem('localOptical')) || [];
					console.log('qqqqq', localOptical);
					console.log('totalOptical', totalOptical);
					//将本地存储数据和总的异常数据（异常光纤和异常井盖）传到getAbnormalChange函数进行比较弹出
					this.getAbnormalChange(localOptical, totalOptical);
					//将最新拿到的总的异常数据存储在本地
					localStorage.setItem('localOptical', JSON.stringify(totalOptical));
				});
		},
		getAbnormalChange(oldArr, newArr) {
			let abnormalArr = [];
			newArr.map((item, index) => {
				let isExist = false;
				oldArr.map((items, key) => {
					if (items.id == item.id) {
						isExist = true;
						return;
					}
			...
			abnormalArr.map(
				function(item, index) {
					setTimeout(
						function() {
							this.$message({
								message: JSON.parse(item.entity).location + '-----发生异常,需及时处理！',
								type: 'warning'
							});
						}.bind(this),
						index * 3500
