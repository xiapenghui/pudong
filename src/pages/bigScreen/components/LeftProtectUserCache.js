...
import userArray from "@baidumap/assets/mock/userTest";
...
let getStorageUsers = () => {
    ...
    let json = window.localStorage.getItem("xprotect_users");
    if (json) {
...
let setStorageUsers = (users) => {
    try {
        ...
        if (json) {
            window.localStorage.setItem("xprotect_users");
        }
...
let loadUserInfo = () => {
    return new Promise((resolve , reject)=>{
        Promise.all([
            ...
            eventBus.emit("app.bzgk.user.refresh", result);
