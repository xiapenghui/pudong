...
import sgdwTable from './baidumap/tables/zyjygkJs/sgdwTable'; // 施工队伍Table
...
export default {
	...
	data() {
		return {
			...
			// 施工队伍echarts data
			constData: [[20, 30, 10, 22], [79, 82, 90, 101]],
			// 施工队伍总数
			constAll: 0,
			sgdwtableColumns: sgdwTable
		};
	},
	...
	methods: {
		...
		onClickNum(obj) {
			...
			dataApi.getsgdwDetail(params).then(result => {
				debugger;
		...
		getCurConstructorStats() {
			dataApi.getsgdwNumber().then(result => {
				let wkresponser = result.data.data.onlineMap.wkresponser;
				let supervisor = result.data.data.onlineMap.supervisor;
				let wkapprover = result.data.data.onlineMap.wkapprover;
				let manage = result.data.data.onlineMap.manage;
				let wkresponser_total = result.data.data.totalMap.wkresponser;
				let supervisor_total = result.data.data.totalMap.supervisor;
				let wkapprover_total = result.data.data.totalMap.wkapprover;
				let manage_total = result.data.data.totalMap.manage;
				...
				this.constData = [[supervisor, manage, wkapprover, wkresponser], [supervisor_total, manage_total, wkapprover_total, wkresponser_total]];
