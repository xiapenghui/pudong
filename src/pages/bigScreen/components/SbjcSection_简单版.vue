...
export default {
	...
	methods: {
		...
		delayed() {
			...
			// 			'获取异常井盖设备列表'
			this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/xczy/tXczyWarnMessage/getTXczyWarnMessageList?rangeType=month&offset=0&limit=5',
						'获取异常井盖设备列表').then(result => {
				...
				this.totalOptical=abnormalWell.concat(abnormalOptical);
			});
		...
		getAbnormalChange(oldArr, newArr) {
			...
			console.log('异常数据', abnormalArr);
			abnormalArr.map(function(item, index) {
				setTimeout(function() {
					this.$message({
						message: JSON.parse(item.entity).location, 
						type: 'warning',
						customClass: 'fixeTop'
					});
	...
	watch:{
		totalOptical(newVal,olVal){
			console.log("olVal",olVal)
			console.log("newVal",newVal)
			this.getAbnormalChange(olVal,newVal)
		}
	},
	mounted() {
