		...
		onClickNum(obj) {
			...
			if (obj == 'fkjc') {
				...
				this.$$utils.axiosRequest('/pdaqfh_war_exploded/f/znjg/getDeviceList?offset=0&limit=-1', '获取智能井盖设备列表').then(result => {
					...
					this.allData = listznjg;
					for (let i = 0; i < listznjg.length; i++) {
						this.allData[i].jgName = listznjg[i].name;
					}
					// 新增和池智能井盖数据搜索
					this.$$utils.axiosRequest('/pdaqfh_war_exploded/f/znjg/getDevices?offset=0&limit=-1', '获取和池智能井盖设备列表').then(result => {
						...
						this.allData = listhcznjg;
						for (let i = 0; i < listhcznjg.length; i++) {
							this.allData[i].jgName = listhcznjg[i].name;
						}
					});
				});

				// 原始智能井盖数据
				this.$$utils.axiosRequest('/pdaqfh_war_exploded/f/znjg/getDeviceList?offset=' + this.offset + '&limit=' + this.pageSize, '获取智能井盖设备列表').then(result => {
					debugger;
					let listznjg = result.data.page.list;
					let oldtotal = result.data.page.count;
					for (let i = 0; i < listznjg.length; i++) {
						this.tableData = listznjg;
						this.tableData[i].jgName = listznjg[i].name;
					}
					// 新增和池智能井盖数据
					this.$$utils
						.axiosRequest('/pdaqfh_war_exploded/f/znjg/getDevices?offset=' + this.offset + '&limit=' + this.pageSize, '获取和池智能井盖设备列表')
						.then(result => {
							debugger;
							let listhcznjg = result.data.page.list;
							let newtotal = result.data.page.count;
							for (let i = 0; i < listhcznjg.length; i++) {
								this.tableData = listhcznjg;
								this.tableData[i].jgName = listhcznjg[i].name;
							}
							this.total = oldtotal + newtotal;
							this.loading = false; //加载中结束隐藏
						});
				});
			} else if (obj == 'ycjg') {
