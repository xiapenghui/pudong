...
<script>
	...
	export default {
		...
		computed: {
		  // troubleOption2(){
		  //   return deepCopy(this.troubleOption);
		  // },
		  // troubleOption3(){
		  //   return deepCopy(this.troubleOption);
		  // },
		...
		methods: {
			tabChange(data, index) {
				...
				if (index === 1) {
					...
					this.renderEchart('echarts-bb-2', this.troubleOption2, this.troubleWeekData);
				}
				if (index === 2) {
					//渲染页面echarts图表---本月echarts
					this.colorNewData = this.colorMonthData
					this.renderEchart('echarts-bb-3', this.troubleOption3, this.troubleMonthData);
				}
			},
			setId(pid, id) {
			...
			colorFn(resData){
				...
				console.log(12345);
				console.log(this.troubleOption);
				console.log(this.troubleOption2);
			}
		},
		mounted() {
