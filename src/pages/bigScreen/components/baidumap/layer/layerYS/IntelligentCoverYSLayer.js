...
// import IntelligentCoverYSData from "./IntelligentCoverYSData";
import {
	GEOMETRY_TYPE_POINT
...
import {
	wgs2bd
} from "../../utils/Convert"; //坐标转换

import * as backApiData from "../../../../../bigScreen/api/backApiData.js";
//演示图层-智能井盖
function IntelligentCoverYSLayer() {
	return {
		data() {
			return {
				jgData1: null, //井盖数据1
		...
		watch: {
			ready: function() {
				...
				this.eventBus.on("layer-clickBut-IntelligentCoverYSLayer", this.clickBut); //监听当前图层二级按钮
				this.eventBus.on("layer-visible-IntelligentCoverYSLayer", this.onLayerVisible); //图层显示隐藏监听
				this.eventBus.on("graphic-flyShowTips-IntelligentCoverYSLayer", this.flyShowTips); //定位tips弹框
		...
		methods: {
			IntelligentCoverYSLayer(list) { //图层渲染
				//let data = list;//IntelligentCoverYSData;//智能井盖数据
				let data = []; //
				if (list.length > 0) { //河池井盖数据
					for (var i = 0; i < list.length; i++) {
						if (list[i].uid == undefined) { //图层唯一标识
							list[i].uid = i;
						}
						data.push(list[i]);
					}
				}
				if (this.jgData1 != null) { //井盖数据1
					for (var i = 0; i < this.jgData1.length; i++) {
						data.push(this.jgData1[i]);
					}
				}
				...
				let gs = [];
				for (let i = 0; i < data.length; i++) {
					if (data[i].lon == undefined || data[i].lon == "" || data[i].lon == 0) {
						continue;
					}
			...
			flyShowTips({
				...
				if (graphicList.length > 0 && layer.id != "") {
			...
			graphicClickHandler({
				...
				let itme = {
					layerName: "IntelligentCoverYSLayer", //图层名称
					title: "智能井盖", //tips标题
					type: "", //tips展示类型
					data: [ //tips数据  井盖名称、所属公司、运维人员、联系方式
						{
							'type': '井盖名称:',
							'detail': graphic.feature.properties.name
						},
						{
							'type': '线路名称:',
							'detail': graphic.feature.properties.channelName
						},
						// { 'type': '所属公司:', 'detail': "上海浦东电力有限公司" },
						{
							'type': '井盖地址:',
							'detail': graphic.feature.properties.address
						},
						// { 'type': '监测状态:', 'detail': "正常" },
					],
			...
			clickBut({
				...
				if (type == "chart") { //查看曲线
					...
					this.$$utils.axiosRequest(MapService.getCollectionDataList + "&deviceId=" + graphic.feature.properties.primaryId +
						"&startTime=" + FrontDay + " 00:00:00&endTime=" + Yesterday + " 23:59:59", '井盖曲线信息').then(result => {
						...
						this.eventBus.emit("graphic-clickTwoMenu-IntelligentCoverChart", {
							itme
						}); //井盖曲线图
					});
				}
			...
			znjgTips() {
				this.$$utils.axiosRequest(MapService.getDeviceList, '智能井盖列表').then(result => {
					...
					this.jgData1 = null;
					this.jgData1 = result.data.page.list;
					let params={
						offset:'0',
						limit:'-1',
						orderBy:'createTime'
					};
					backApiData.getznjg(params).then(result=>{
					    this.IntelligentCoverYSLayer(result.data.data.page.list);
					})
				});
			}
		},
...
export default VectorLayer.extend(IntelligentCoverYSLayer());
