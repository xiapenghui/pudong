...
function PowerStationLayer10kv() {//电站-10kv
    return {
        data() {
            return {};
        },
        ...
        watch: {
            ready: function () {
                ...
                this.eventBus.on("graphic-flyShowTips-PowerStationLayer10kv", this.flyShowTips);//定位tips弹框
        ...
        methods: {
            PowerStation(list) {//图层渲染
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (data[i].voltage == "交流10kV" || data[i].voltage == "直流10kV") {
                            ...
                            new Graphic({
                                ...
                                feature: new Feature({
                                    ...
                                    geometry: new Point({
                                        ...
                                        coordinates: [data[i].jd, data[i].wd]// [data[i].longitude,data[i].latitude]
                                    }),
            ...
            graphicClickHandler({ pixel, point, graphic, map }) {//点击tips信息
                let itme = {
                    layerName: "PowerStationLayer10kv",//图层名称
                    title: "电站",//tips标题
                    type: "",//tips展示类型
                    data: [//tips数据 
        ...
        created() {
            this.$$utils.axiosRequest(MapService.getTAqfhStationInfoList, '电站列表').then(result => {
                ...
                this.eventBus.emit("layer-data-PowerStationLayer35kvService", result.data.list);//35kv图层渲染
                this.eventBus.emit("layer-data-PowerStationLayer110kvService", result.data.list);//110kv图层渲染
            });
        }
    };
};
...
export default VectorLayer.extend(PowerStationLayer10kv());
