...
function ProtectionObjectBdLayer() {//保电对象-东郊宾馆
    return {
        data() {
            return {
            };
        ...
        watch: {
            ready: function () {
                ...
                this.eventBus.on("layer-visible-ProtectionObjectBdLayer", this.onLayerVisible);//图层显示隐藏监听
                this.eventBus.on("layer-clickBut-ProtectionObjectBdLayer", this.clickBut);//监听当前图层二级按钮
                this.eventBus.on("graphic-flyShowTips-ProtectionObjectBdLayer", this.flyShowTips);//定位tips弹框
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (data[i].properties.ID != "BD2018080907") {
                        continue;
                    }
                    gs.push(
                        new Graphic({
        ...
        methods: {
            ...
            graphicClickHandler({ pixel, point, graphic, map }) {//点击tips信息
                ...
                let itme = {
                    layerName: "ProtectionObjectBdLayer",//图层名称
                    title: "保障对象",//tips标题
                    type: "",//tips展示类型
...
export default VectorLayer.extend(ProtectionObjectBdLayer());
