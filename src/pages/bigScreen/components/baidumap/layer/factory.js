...
import IntelligentCoverBdLayer from "./IntelligentCoverBdLayer" //智能井盖
...
import ChannelMonitoringLayer from "./ChannelMonitoringLayer" //通道监拍
import ChannelMonitoringBdLayer from "./ChannelMonitoringBdLayer" //通道监拍-保电
import OpenDialog1Layer from "./OpenDialog1Layer" //弹框图层1
...
import ProtectionObjectBdLayer from "./ProtectionObjectBdLayer" //保电对象-东郊宾馆
import FlyPointBdLayer from "./FlyPointBdLayer" //定位坐标点
...
import GridPersonnelHeatLayer from "./heat/GridPersonnelHeatLayer" //网格化运维人员热力图
...
export default (options) => {
    ...
    layer.$data.delay = options.delay || 0;
...
function instance(className) {
    ...
    if ("IntelligentCoverBdLayer" === className) {//智能井盖-保电
        return new IntelligentCoverBdLayer();
    }
    ...
    if ("ChannelMonitoringLayer" === className) {//通道监拍
        return new ChannelMonitoringLayer();
    }
    if ("ChannelMonitoringBdLayer" === className) {//通道监拍-保电
        return new ChannelMonitoringBdLayer();
    }
    if ("OpenDialog1Layer" === className) {//弹框图层1
        return new OpenDialog1Layer();
    }
    if ("ProtectionObjectLayer" === className) {//保电对象
        return new ProtectionObjectLayer();
    }
    if ("ProtectionObjectBdLayer" === className) {//保电对象-东郊宾馆
        return new ProtectionObjectBdLayer();
    }
    if ("FlyPointBdLayer" === className) {//坐标定位点
        return new FlyPointBdLayer();
    }
    ...
    if ("GridPersonnelHeatLayer" === className) {//网格化运维人员热力图 
        return new GridPersonnelHeatLayer();
    }
}