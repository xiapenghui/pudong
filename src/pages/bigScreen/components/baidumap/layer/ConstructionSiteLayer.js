...
function ConstructionSiteLayer() {//施工现场
    return {
        ...
        methods: {
            ...
            loadPerson(type, alias) {
                return new Promise((resolve, reject) => {
                    this.$$utils.axiosRequest(
                        MapService.getOnlineConstructorList + "&personType=" + type,
                        alias
                    ).then(result => {
                        if (result.data.list == 0) {
                            Console.debug(alias + " : 0");
                            resolve([]);
                        }

                        Console.debug(alias + " : " + result.data.list.length);
            ...
            loadData: function () {
                Promise.all([
                    this.loadPerson("supervisor", "施工现场-查岗人员"),
                    this.loadPerson("manage", "施工现场-管理人员"),
                    this.loadPerson("wkapprover", "施工现场-工作许可人"),
                    this.loadPerson("wkresponser", "施工现场-工作负责人"),
                ]).then(
                    function (responses) {
                        this.removeAll();
