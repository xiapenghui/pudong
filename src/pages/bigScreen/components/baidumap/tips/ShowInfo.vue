<template>
  <div class="offsetBox" :style="{left:newOffset.x+'px',top:newOffset.y+'px'}">
     <div class="boxinfo" :style="{width:newWidth+'px',height:newHeight+'px',top:top+'px',left:left+'px'}" v-show="isShow==true">
  ...
  data() {
   ...
   methods: {
	...
	showTips({pixel, point, graphic,map,itme }){//打开tips
		...
		this.newMap = map;
		this.newPoint = point;
		if(itme.layerName=="OpenDialog1Layer"){
			this.newWidth=310;
		}else{
			this.newWidth=375;
		}
		this.left = pix.x-(this.newWidth/2);
		...
		if(this.title=='指挥中心' || this.title=='电缆振动'){//tips不带按钮
			...
			this.newHeight = 250;
			this.top=pix.y-this.height;
		}else if(this.title=='陆家嘴高可靠性示范区'||this.title=='张江科学城能源互联网示范区'){
			this.detailHeight = 180;
			this.newHeight = 260;
			this.top=pix.y-this.height;
		}else{
