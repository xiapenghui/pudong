...
export default {
    ...
    return {
      ...
      piclist: [
        ...
        },
        {
          ...
          controlName: ["ConstructionSiteLayer"]
        },
         {
          isHover: true,
          url: require("../assets/images/ic_construction_sel.png"),
          ...
          title: "工作票",
          class: "build",
          layerName: "WorkTicketLayer",
          controlName: ["WorkTicketLayer"]
        },
        {
          isHover: true,
        ...
        {
          isHover: true,
          url: require("../assets/images/ic_video_sel.png"),
          hover: require("../assets/images/ic_video.png"),
          title: "通道监拍",
          class: "cable",
          layerName: "ChannelMonitoringLayer",
          controlName: ["ChannelMonitoringLayer"]
        },
        {
          isHover: true,
          url: require("../assets/images/ic_videoBd.png"),
          hover: require("../assets/images/ic_videoBd_sel.png"),
          title: "通道监拍-保电",
          class: "cable",
          layerName: "ChannelMonitoringBdLayer",
          controlName: ["ChannelMonitoringBdLayer"]
        },
        {
          isHover: true,
          url: require("../assets/images/ic_bdCover.png"),
          hover: require("../assets/images/ic_bdCover_sel.png"),
          title: "保电井盖",
          class: "cable",
          layerName: "IntelligentCoverBdLayer",
          controlName: ["IntelligentCoverBdLayer"]
        }
      ],
      vmapD: "",
      mapD: "",
  ...
  methods: {
    ...
    switchLayer(type) {
      ...
      if (type == "综合监控") {
        //专题1
        //canvasRender.default.eventBus.emit("canvas-renderer-fly", { x: 121.659898, y: 31.132255, zoom: 12 });//定位
        ...
        for (var i = 0; i < piclist.length; i++) {
          if (
            ...
            if (
              ...
              piclist[i].layerName == "IntelligentCoverBdLayer" ||
              piclist[i].layerName == "ChannelMonitoringBdLayer"
            ) {
        ...
        eventBus.emit("layer-visible-GridPersonnelHeatLayer", false); //网格化运维人员热力图
        eventBus.emit("layer-visible-ProtectionObjectBdLayer", false); //保电对象-东郊宾馆

        var layerList = canvasRender.default.layers;
        for (var i = 0; i < layerList.length; i++) {
          if (layerList[i].$data.name == "OpenDialog1Layer") {
            layerList[i].openDilog1(); //弹框图层1
          }
        }
      } else if (type == "安全管控") {
        canvasRender.default.eventBus.emit("canvas-renderer-fly", {
        ...
        for (var i = 0; i < piclist.length; i++) {
          if (
            ...
            piclist[i].layerName == "WorkTicketLayer" ||
        ...
        eventBus.emit("graphic-ShowInfo-closeTips"); //关闭弹框图层
        eventBus.emit("layer-visible-GridPersonnelHeatLayer", true); //网格化运维人员热力图
        eventBus.emit("layer-visible-ProtectionObjectBdLayer", false); //保电对象-东郊宾馆
      } else if (type == "智慧保电") {
        canvasRender.default.eventBus.emit("canvas-renderer-fly", {
          x: 121.659898,
        ...
        for (var i = 0; i < piclist.length; i++) {
          if (
            ...
            if (
              ...
              piclist[i].isHover = false; //显示图层控制
            } else {
              piclist[i].isHover = true; //显示图层控制
            }
            for (var c = 0; c < piclist[i].controlName.length; c++) {
        ...
        //     layers[i].visible = false; //隐藏专题图层
        //  }

        eventBus.emit("graphic-ShowInfo-closeTips"); //关闭弹框图层
        eventBus.emit("layer-visible-GridPersonnelHeatLayer", false); //网格化运维人员热力图
        eventBus.emit("layer-visible-ProtectionObjectBdLayer", true); //保电对象-东郊宾馆
      }
    }
  },
  mounted() {
