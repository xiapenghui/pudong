						...
						<div class="detailContentMove" ref="detailContentMove">
							<div ref="detailContext" class="detailContext">
								<div v-for="(item, index) in detailTableData" :key="index" class="detailContextItemWrap">
									<div class="detailContextItem" @mouseover="selectStyle" @mouseout="outStyle">
...
export default {
	...
	methods: {
		...
		onClickNum(obj) {
			...
			this.$$utils
				.axiosRequest(
					'/pdaqfh_Web_exploded/f/sggk/getWorkTicketList?rangeType=' +
						rangeType +
						'&ticketType=' +
						ticketType +
						'&status=' +
	...
	mounted() {
		// 今日工作票
		this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketSummary?rangeType=today&status=1', '按时间和类型统计工作票数量').then(result => {
			if (result.data == null) {
				this.workingTodayData[0] = [0, 0, 0, 0, 0, 0];
			} else {
				this.workingTodayData[0] = [result.data['6'], result.data['5'], result.data['4'], result.data['3'], result.data['2'], result.data['1']];
			}

			this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketSummary?rangeType=today&status=0', '按时间和类型统计工作票数量').then(result => {
				if (result.data == null) {
					this.workingTodayData[1] = [0, 0, 0, 0, 0, 0];
				} else {
					this.workingTodayData[1] = [result.data['6'], result.data['5'], result.data['4'], result.data['3'], result.data['2'], result.data['1']];
				}
		...
		// 本周工作票
		this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketSummary?rangeType=week&status=1', '按时间和类型统计工作票数量').then(result => {
			if (result.data == null) {
				this.workingWeekData[0] = [0, 0, 0, 0, 0, 0];
			} else {
				this.workingWeekData[0] = [result.data['6'], result.data['5'], result.data['4'], result.data['3'], result.data['2'], result.data['1']];
			}

			this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketSummary?rangeType=week&status=0', '按时间和类型统计工作票数量').then(result => {
				this.workingWeekData[1] = [result.data['6'], result.data['5'], result.data['4'], result.data['3'], result.data['2'], result.data['1']];
			});
		});

		// 本月工作票
		this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketSummary?rangeType=month&status=1', '按时间和类型统计工作票数量').then(result => {
			/* this.workingMonthData[0] = [result.data['6'], result.data['5'], result.data['4'], result.data['3'], result.data['2'], result.data['1']];
				this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketSummary?rangeType=month&status=0',
					...
					this.workingMonthData[1] = [result.data['6'], result.data['5'], result.data['4'], result.data['3'], result.data['2'], result.data['1']];
				}); */
			if (result.data == null) {
			...
			this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketSummary?rangeType=month&status=0', '按时间和类型统计工作票数量').then(result => {
				this.workingMonthData[1] = [result.data['6'], result.data['5'], result.data['4'], result.data['3'], result.data['2'], result.data['1']];
			});
		});

		// 获取工作票跑马灯数据
		this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketList?rangeType=today&ticketType=-1&limit=-1&offset=0&status=0', '获取工作票汇总列表').then(result => {
		...
		//超时开工、延时汇报、处理中、已完成
		this.$$utils
			.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketList?rangeType=today&ticketType=-1&limit=-1&offset=0&status=888', '获取工作票汇总列表--超时开工')
			.then(result => {
		...
		this.$$utils
			.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketList?rangeType=today&ticketType=-1&limit=-1&offset=0&status=999', '获取工作票汇总列表--延时汇报')
			.then(result => {
		...
		this.$$utils
			.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketList?rangeType=today&ticketType=-1&limit=-1&offset=0&status=0', '获取工作票汇总列表--处理中')
			.then(result => {
				this.clz = result.data.list.length;
			});
		this.$$utils
			.axiosRequest('/pdaqfh_Web_exploded/f/sggk/getWorkTicketList?rangeType=today&ticketType=-1&limit=-1&offset=0&status=1', '获取工作票汇总列表--已完成')
			.then(result => {
