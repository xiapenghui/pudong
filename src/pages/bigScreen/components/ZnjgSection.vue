<template>
	<div :style="{ width: width + 'px', height: height + 'px', top: top + 'px', left: left + 'px' }" v-show="show">
		<div class="sectionTop"><div class="sectiontitle">智能井盖</div></div>
		<div class="sectionIn">
			<div class="sectionInBody">
				<ul ref="articleUl" class="articleUl">
					<li :style="{ width: itemWidth + 'px' }" class="articleLi" v-for="(items, index) in msgData">
						<div class="articleItem articleHeightItem" @click="onFun(index)" @mouseover="onOver(index)" @mouseleave="onLeave(index)">
							<div class="articleItemIn"><deviceDetail :device-data="items" /></div>
						</div>
					</li>
				</ul>
