<template>
	<div id="pdaqfhVueApp" v-cloak>
		<div class="homePage">
			<pageHeader />
			<sbjcSection v-show="homePage && controlShow && allShow" class="section" />
			<!-- <sbjsSection v-show="homePage && controlShow allShow" class="section"/> -->
			<xsqxSection v-show="homePage && controlShow && allShow" class="section" ref="xsqx" />
			<xsyhSection v-show="homePage && controlShow && allShow" class="section" />
			<jpzzSection v-show="homePage && controlShow && allShow" class="section" />
			<mapSection class="mapWrap" />
			<xczySection v-show="homePage && !controlShow && allShow" class="section" />
			<zyjkSection v-show="homePage && !controlShow && allShow" class="section" />
			<!-- 暂时隐藏中控 -->
			...
			<!-- 智能保电模块 -->
			<bzgkSection v-show="bzgkShowOff" class="section offSection" />
			<bzgkListSection v-show="bzgkShowOff" class="section" ref="leftListSection" />
			<bzrwSection v-show="bzgkShowOff" class="section" ref="listSection" />
			<!-- 智能保电弹窗框 -->
			<znbdDetailed v-show="bzgkShow" class="znbdTc" @onhand-click="onhandClick" />
			<!-- 汇总信息 -->
			<messageTotal v-show="bzgkShowOff" ref="messageSection" />
		</div>
	</div>
</template>
...
// 左侧上方保障概况保障任务
import bzgkSection from '../../components/BzgkSection.vue';
//左侧下方保障概况保障方块列表
import bzgkListSection from '../../components/BzgkListSection.vue';
// 右侧保障任务
import bzrwSection from '../../components/BzrwSection.vue';
//智能保电弹出框
import znbdDetailed from '../../components/ZnbdDetailed.vue';
// 汇总信息
import messageTotal from '../../components/messageTotal.vue';
	...
	methods: {
		...
		msgInterval(ele, ele2, timeSplit = 3000) {
			if (ele && ele2) {
				...
				let eleSize = _this.getStyle(ele.children[0], 'height') + _this.getStyle(ele.children[0], 'paddingTop') + _this.getStyle(ele.children[0], 'paddingBottom');
				// 初始化元素2的top值
		...
		setWidth(ele, ele2) {
			let _this = this;
			let len = ele.children.length;
			let eleSize = _this.getStyle(ele.children[0], 'width') + _this.getStyle(ele.children[0], 'paddingLeft') + _this.getStyle(ele.children[0], 'paddingRight');
		...
		// 保障状态按钮
		bdStstusParent() {
			this.bzgkShow = !this.bzgkShow;
		},
		onhandClick(row) {
			this.bzgkShowOff = true;
			this.allShow = false;
			this.zkCenter = false;
			this.bzgkShow = false;
			// this.$refs.listSection.perTimeWatch()

			// this.$refs.messageSection.scrollTop()
			// console.log("aa",this.$refs.listSection.perTimeWatch())
			//保障对象
			let layer = {
				id: 'BD2018080907', //primaryId, //定位对象id
...
// 消息推送弹出框样式
body /deep/.el-message--success,
body /deep/.el-message--warning {
	background-color: rgba(30, 91, 145, 0.8) !important;
	height: 120px !important;
	border-color: transparent !important;
}
body /deep/.el-message--success .el-message__content,
body /deep/.el-message--warning .el-message__content {
	font-size: 20px !important;
	color: #fff !important;
}
body /deep/ .rightMessageMask {
	top: 150px !important;
}
</style>
