		...
		<div class="sectionMain">
			<div class="sectionHead">
				<div class="sectionTop">
					<div class="sectiontitle">设备感知</div>
				</div>
			</div>
			<div class="sectionBody sectionBody1">
				<div class="sectionItem">
					<div class="sbgzbox">
						...
						<div class="infobox">
							<div class="toptext">辅控监测(套) <span class="inforight"><strong @click="onClickNum('fkjc')">{{ deviceDetails.fkjc.total }}</strong></span></div>
							<div class="bottomtext">电站(个)<span class="inforight">{{ deviceDetails.fkjc.num1 }}</span></div>
						</div>
					</div>
					<div class="deviceOn">
				...
				<div class="sectionItem" :class="{defult:isdefult}">
					<div class="sbgzbox">
						...
						<div class="infobox">
							<div class="toptext">机器人(套) <span class="inforight"><strong @click="onClickNum('jqr')">{{ deviceDetails.jqr.total }}</strong></span></div>
							<div class="bottomtext">电站(个)<span class="inforight">{{ deviceDetails.jqr.num1 }}</span></div>
						</div>
					</div>
					<div class="deviceOn">
						<div class="deviceOnText">在线率</div>
				...
				<div class="sectionItem">
					<div class="sbgzbox">
						...
						<div class="infobox">
							<div class="toptext">消防设备(套) <span class="inforight"><strong>{{ deviceDetails.xfsb.equipmentCount}}</strong></span></div>
							<div class="bottomtext">电站(个)<span class="inforight">{{ deviceDetails.xfsb.onlineCount }}</span></div>
						</div>
					</div>
					<div class="deviceOn">
						<div class="deviceOnText">在线率</div>
				...
				<div class="sectionItem" :class="{rounds:round}">
					<div class="sbgzbox">
						...
						<div class="infobox">
							<div class="toptext">视频监控(套) <span class="inforight"><strong @click="onClickNum('spjk')">{{ deviceDetails.spjk.total }}</strong></span></div>
							<div class="bottomtext">电站(个)<span class="inforight">{{ deviceDetails.spjk.onlineNum }}</span></div>
						</div>
					</div>
					<div class="deviceOn">
						<div class="deviceOnText">在线率</div>
				...
				<div class="sectionItem" :class="{rounds:round}">
					<div class="sbgzbox">
						...
						<div class="infobox">
							<div class="toptext">线缆断线(套) <span class="inforight"><strong @click="onClickNum('xldx')">{{ deviceDetails.xldx.deviceNum }}</strong></span></div>
							<div class="bottomtext">架空线(条)<span class="inforight">{{ deviceDetails.xldx.onlineNum }}</span></div>
						</div>
					</div>
					<div class="deviceOn">
						<div class="deviceOnText">在线率</div>
				...
				<div class="sectionItem" :class="{rounds:round}">
					<div class="sbgzbox">
						...
						<div class="infobox">
							<div class="toptext">光纤振动(套) <span class="inforight"><strong @click="onClickNum('gxzd')">{{ deviceDetails.gqzd.Device_num }}</strong></span></div>
							<div class="bottomtext">电缆(条)<span class="inforight">{{ deviceDetails.gqzd.Online_num }}</span></div>
						</div>
					</div>
					<div class="deviceOn">
						<div class="deviceOnText">在线率</div>
				...
				<div class="sectionItem" :class="{rounds:round}">
					<div class="sbgzbox">
						...
						<div class="infobox">
							<div class="toptext">智能井盖(个) <span class="inforight"><strong @click="onClickNum('znjg')">{{ deviceDetails.znjg.manholeCover }}</strong></span></div>
							<div class="bottomtext">工井(个)<span class="inforight">{{ deviceDetails.znjg.deviceTotal }}</span></div>
						</div>
					</div>
					<div class="deviceOn">
						<div class="deviceOnText">在线率</div>
				...
				<div class="sectionItem">
					<div class="sbgzbox">
						...
						<div class="infobox">
							<div class="toptext">HPLC<span class="inforight"><strong @click="onClickNum('hplc')">{{ deviceDetails.hplc.total }}</strong></span></div>
							<div class="bottomtext">用户(户)<span class="inforight">{{ deviceDetails.hplc.num1 }}</span></div>
						</div>
					</div>
					<div class="deviceOn">
						<div class="deviceOnText">在线率</div>
				...
				<!-- 设备感知弹窗 -->
				<detail-info :columns="tableColumns" :dataSource="tableData" :currentPage="currentPage" :hasIndex="true" :isShow="isShow" :isSearch="isSearch"
				 :total="total" :pageSize="pageSize" :handTitle="handTitle" :loading="loading" @showClose="onClose" @current-change="onCurrentChange"
				 @onhand-click="onhandClick" @on-input="onInput"></detail-info>
	...
	export default {
			...
			return {
				...
				znjgColumns: [{
					...
					},
					{
						...
						label: '所属公司', //<String>   表头标签
					},
					{
						hasSort: false, //<Boolean> 是否排序
						isCustomize: false, //<Boolean> 是否自定义
						prop: 'people', //<String>  对应属性名
						label: '运维人员', //<String>   表头标签
					},
					{
						hasSort: false, //<Boolean> 是否排序
						isCustomize: false, //<Boolean> 是否自定义
				...
				if (obj == 'fkjc') {
				   ...
				   if(active=='none'){
					...
					this.$$utils.axiosRequest(
						'/pdaqfh_war_exploded/f/sjafpt/monitorDevice?systemCode=dydldx&orderBy=name&serviceCode=dydldx_002&offset=' +
						this.offset + '&limit=' + this.pageSize, '获取索骏安防平台监测设备列表--线缆断线明细').then(result => {
					...
					this.loading = true;
					this.$$utils.axiosRequest(
						'/pdaqfh_war_exploded/f/dasgxczd/getOnlineDevice?offset=' +
						this.offset + '&limit=' + this.pageSize, '获取DAS当前系统状态--光纤振动').then(result => {
						console.log('获取DAS当前系统状态--光纤振动', result)
						let list = result.data.page.list;
						this.total = result.data.page.count
						for (let i = 0; i < list.length; i++) {
							this.tableData = list;
					...
					this.$$utils.axiosRequest('/pdaqfh_war_exploded/f/znjg/getDeviceList?offset=' + this.offset + '&limit=' + this.pageSize,
						'获取智能井盖设备列表').then(result => {
						let list = result.data.page.list;
						this.total = result.data.page.count
						for (let i = 0; i < list.length; i++) {
		...
		mounted() {
			...
			this.$$utils.axiosRequest(OnlineUrl.getznjg, '获取智能井盖数据状态统计数据').then(result => {
				...
				this.deviceDetails.znjg = result.data;
			});
			
			// 获取消防设备接口(p_f_016)
			this.$$utils.axiosRequest(OnlineUrl.getxfsb, '获取智能井盖数据状态统计数据').then(result => {
		        // console.log('获取智能井盖数据状态统计数据',result)
				this.deviceDetails.xfsb = result.data;
			});
			
			// 获取异常数据的接口
			this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/xczy/tXczyWarnMessage/getTXczyWarnMessageList?offset=0&limit=-1', '获取今日告警消息列表').then(result => {
				// this.deviceDetails.znjg = result.data;
				// console.log('告警数据',result)
			});
