				...
				<div class="sourceBox">
					<div class="subSourceBox">
						<div class="subSourceHead">
							<span class="subTitle">4G单兵</span><span class="titleTail"></span><span class="subText">总数</span><span class="subNum">0</span><span
							 class="subText">在线数</span><span class="subNum">0</span>
						</div>
					...
					<div class="subSourceBox">
						<div class="subSourceHead">
							<span class="subTitle">AR眼镜</span><span class="titleTail"></span><span class="subText">总数</span><span class="subNum">0</span><span
							 class="subText">在线数</span><span class="subNum">0</span><span class="floatRight">待开发</span>
						</div>
						<div class="subSourceBody">
							<ul class="sourceUl">
								<li class="sourceLi">
...
<script>
	...
	export default {
			...
			return {
				...
				tableColumns: [{
					...
					},
					{
						...
						prop: 'user_name', //<String>  对应属性名
						label: '用户名称', //<String>   表头标签
					},
					{
						hasSort: false, //<Boolean> 是否排序
						isShow: true, //<Boolean> 是否展示
						prop: 'dept_name', //<String>  对应属性名
						label: '专业名称', //<String>   表头标签
					},
					{
						hasSort: false, //<Boolean> 是否排序
						isShow: true, //<Boolean> 是否展示
		...
		mounted() {
			...
			this.$$utils.axiosRequest('/pdaqfh_Web_exploded/f/wgyw/getOnlinePeopleLocations?limit=-1', '获取当前人员统计信息').then(result => {
				let xlCount=0;
				let bdCont=0;
				let dlCont=0;
				let pdCont=0;
				for(let i=0;i<result.data.length;i++){
					let type=result.data[i].dept_name;
					if(type=="线路"){						
				     	xlCount++;	
						this.xianLu.push(result.data[i])
					}else if(type=="变电"){
						bdCont++;
						this.bianDian.push(result.data[i])
					}else if(type=="电缆"){
						dlCont++;
						this.dianLan.push(result.data[i])
					}else if(type=="配电"){
						pdCont++;
						this.peiDian.push(result.data[i])
					}
				}
